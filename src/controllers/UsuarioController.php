...
class UsuarioController
{
    ...
    public function indexGeral()
    {
        ...
        return [
            'view' => './src/views/usuario/index-geral.php',
            'data' => ['usuarios' => $users]
        ];    
    }
    ...
    public function delete()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Validação de segurança para garantir que o ID é número
            ...
            if ($id) {
                ...
                if ($id == $_SESSION['usuario_id']) { // se o id que deseja deletar for o mesmo da sessão atual
                    if($this->usuario->delete()){
                        session_destroy();
                        header('Location: index.php?action=welcome');
                        exit;
                    }
                } elseif ($this->usuario->delete()) {
                    $_SESSION['mensagem'] = [
                        'tipo' => 'sucesso', 
                ...
                $_SESSION['mensagem'] = [
                    'tipo' => 'erro', 
                    'texto' => 'ID de usuário inválido.'
                ];
            }

            header('Location: index.php?action=index-geral'); 
            exit;
        }

        // Se tentarem acessar essa função sem ser via POST, joga para a home/lista
        header('Location: index.php?action=index-geral');
        exit;
    }

    public function readuser(int $id = 0){

        $this->usuario->id = $id;

            if ($this->usuario->read()) {
                $usuario = [
                    'id' => $this->usuario->id,
                ...
                return [
                    'view' => './src/views/usuario/read.php',
                    'data' => ['usuario' => $usuario]
                ];
            }
        return [
            'view' => './src/views/usuario/index-geral.php',
            'data' => ['usuario' => $this->usuario->getAll(), 'erro' => 'Usuário não encontrado.']
        ];
